...
export default function Contacts() {
  ...
  function getAuditBadge(cid) {
    ...
    if (s >= 70) return { color: '#34d399', bg: 'rgba(52,211,153,0.15)', label: s, icon: '🟢' }
    if (s >= 40) return { color: '#f59e0b', bg: 'rgba(245,158,11,0.15)', label: s, icon: '🟡' }
    return { color: '#f87171', bg: 'rgba(248,113,113,0.15)', label: s, icon: '🔴' }
  }
    ...
    <div>
      ...
      {auditLoading && (
        <div style={{ ...card, padding: '0.85rem 1.25rem', marginBottom: '1rem', borderLeft: '4px solid #D4AF37', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div style={{ fontSize: '0.85rem', color: '#D4AF37', fontWeight: 600 }}>🔍 Audit automatique en cours... Recherche d'informations et d'actualités</div>
          <style>{'@keyframes spin { to { transform: rotate(360deg) } }'}</style>
        </div>
      )}
      ...
      <div style={{ ...card, padding: '1.5rem 2rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem' }}>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fff', margin: 0 }}>👥 Contacts & Clients</h2>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.3rem' }}>
            {[{ id: 'all', label: 'Tous (' + contacts.length + ')' }, { id: 'companies', label: '🏢 Entreprises (' + totalCompanies + ')' }, { id: 'people', label: '👤 Personnes (' + totalPeople + ')' }].map(f => (
                    ...
                    <tr key={c.id} onClick={() => setSelectedContact(isSelected ? null : c)} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', transition: 'background 0.15s', background: isSelected ? 'rgba(212,175,55,0.08)' : 'transparent' }} onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'rgba(212,175,55,0.04)' }} onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = 'transparent' }}>
                      <td style={{ padding: '0.85rem 1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: c.is_company ? 'rgba(96,165,250,0.15)' : 'rgba(212,175,55,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', flexShrink: 0 }}>{c.is_company ? '🏢' : '👤'}</div>
                      ...
                      <td style={{ padding: '0.85rem 1rem' }}>
                        <div style={{ display: 'flex', gap: '0.3rem' }} onClick={e => e.stopPropagation()}>
                          ...
                          <button onClick={() => handleDelete(c.id)} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', color: '#f87171', width: '32px', height: '32px', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  )
          ...
          return (
            <div style={{ ...card, padding: '1.5rem', position: 'sticky', top: '80px' }}>
              ...
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '1rem', border: '1px solid rgba(255,255,255,0.04)' }}>
                {selectedContact.email && <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><span style={{ fontSize: '0.85rem' }}>📧</span><a href={'mailto:' + selectedContact.email} style={{ color: '#60a5fa', fontSize: '0.82rem', textDecoration: 'none' }}>{selectedContact.email}</a></div>}
                {selectedContact.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}><span style={{ fontSize: '0.85rem' }}>📞</span><a href={'tel:' + selectedContact.phone} style={{ color: '#e2e8f0', fontSize: '0.82rem', textDecoration: 'none' }}>{selectedContact.phone}</a></div>}
              ...
              {!audit ? (
                <div style={{ marginBottom: '1rem' }}>
                  <button onClick={() => runAutoAudit(selectedContact.id, selectedContact.is_company ? selectedContact.name : selectedContact.company)} disabled={auditLoading || (!selectedContact.company && !selectedContact.is_company)} style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', background: 'rgba(212,175,55,0.1)', border: '1px solid rgba(212,175,55,0.2)', color: '#D4AF37', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600 }}>🔍 Lancer l'audit de l'entreprise</button>
                </div>
              ) : (
                <>
                  {analysis && (
                    <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '1rem', border: '1px solid rgba(255,255,255,0.04)' }}>
                      <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#D4AF37', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>🏦 Audit entreprise</div>
                      ...
                      {analysis.capacite_budget_it && <div style={{ fontSize: '0.75rem', color: '#D4AF37', marginTop: '0.3rem' }}>💰 Budget IT estimé : {analysis.capacite_budget_it}</div>}
                      {analysis.tjm_max_estime && <div style={{ fontSize: '0.75rem', color: '#60a5fa', marginTop: '0.2rem' }}>📊 TJM max : {analysis.tjm_max_estime}€/j</div>}
                      {analysis.resume_actualites && <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.4rem', lineHeight: 1.4, borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: '0.4rem' }}>📰 {analysis.resume_actualites}</div>}
                    </div>
                  )}
                  ...
                  {news.length > 0 && (
                    <div style={{ marginBottom: '1rem' }}>
                      <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#60a5fa', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>📰 Actualités ({news.length})</div>
                      {news.slice(0, 4).map((n, i) => (
                        <a key={i} href={n.url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', padding: '0.5rem 0.6rem', borderRadius: '8px', marginBottom: '0.3rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.04)', textDecoration: 'none', transition: 'background 0.15s' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.2rem' }}>
                            <span style={{ fontSize: '0.65rem', color: n.is_it_source ? '#60a5fa' : '#4a6370' }}>{n.is_it_source ? '💻' : '📰'} {n.source}</span>
              ...
              {/* Opportunities */}
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#D4AF37', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>💼 Opportunités ({getContactOpps(selectedContact.id).length})</div>
              ...
              {/* Interactions */}
              <div>
                <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#60a5fa', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>📞 Dernières interactions ({getContactInteractions(selectedContact.id).length})</div>
                {getContactInteractions(selectedContact.id).length === 0 ? (
                  ...
                  <div key={int.id || i} style={{ padding: '0.5rem 0.6rem', borderRadius: '8px', marginBottom: '0.3rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.04)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.78rem', color: '#e2e8f0' }}>{int.type === 'appel' ? '📞' : int.type === 'email' ? '📧' : int.type === 'reunion' ? '🤝' : '📝'} {int.type}</span>
              ...
              <div style={{ display: 'flex', gap: '0.4rem', marginTop: '1rem' }}>
                ...
                {selectedContact.email && <a href={'mailto:' + selectedContact.email} style={{ flex: 1, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.2)', color: '#34d399', padding: '0.5rem', borderRadius: '8px', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, textDecoration: 'none', textAlign: 'center', display: 'block' }}>📧 Email</a>}
                {selectedContact.phone && <a href={'tel:' + selectedContact.phone} style={{ flex: 1, background: 'rgba(212,175,55,0.1)', border: '1px solid rgba(212,175,55,0.2)', color: '#D4AF37', padding: '0.5rem', borderRadius: '8px', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, textDecoration: 'none', textAlign: 'center', display: 'block' }}>📞 Appeler</a>}
              </div>
            </div>
          )
        })()}
          ...
          <div style={{ ...card, width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', padding: '2rem', boxShadow: '0 24px 64px rgba(0,0,0,0.4)' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
                <button type="button" onClick={() => setFormData({ ...formData, is_company: false })} style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: 'none', cursor: 'pointer', background: !formData.is_company ? 'rgba(212,175,55,0.15)' : 'rgba(255,255,255,0.05)', color: !formData.is_company ? '#D4AF37' : '#64808b', fontWeight: !formData.is_company ? 600 : 400, fontSize: '0.85rem' }}>👤 Personne</button>
                <button type="button" onClick={() => setFormData({ ...formData, is_company: true })} style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: 'none', cursor: 'pointer', background: formData.is_company ? 'rgba(96,165,250,0.15)' : 'rgba(255,255,255,0.05)', color: formData.is_company ? '#60a5fa' : '#64808b', fontWeight: formData.is_company ? 600 : 400, fontSize: '0.85rem' }}>🏢 Entreprise</button>
              </div>
              <div style={{ marginBottom: '1rem' }}>
